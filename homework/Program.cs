...
void FindStringSum(int[,] array){//--МЕТОД КОТОРЫЙ ПРОСТО КРАСИВО ВЫВОДИТ СУММУ СТРОКИ  (В задаче ==> 2)                                                                                                    
    int[] summString = new int[array.GetLength(0)]; //МАССИВА РЯДОМ С САМОЙ СТРОКОЙ
    int sum;                                        //ПРОСТО НАПИСАЛ ТАКОЙ ДЛЯ КРАСОТЫ)
    for(int i = 0; i < array.GetLength(0); i++){    //(МЕТОД НЕОБЯЗАТЕЛЬНЫЙ)
        sum = 0;
        for(int j = 0; j < array.GetLength(1); j++){
            sum += array[i, j];
        }
...
void FindMinStringSum(int[,] array){//НАХОДИТ ИНДЕКС СТРОКИ С НАИМЕНЬШЕЙ СУММОЙ ЭЛЕМЕНТОВ (В задаче ==> 2)
    ...
    for(int i = 0; i < array.GetLength(0); i++){   //ЭТОТ МЕТОД ПОКАЖЕТ ОБА ИНДЕКСА СТРОК
        ...
        for(int j = 0; j < array.GetLength(1); j++){//НО Я РЕШИЛ ЧЕРЕЗ STRING (поработать с конвертацией).   
            sum += array[i, j];
        }
        summString[i] = sum;
    } 
